...
function HexGrid({ grid, size, onCellClick, selectionMode = 'fill', orangeColor, greenColor, themeId }: HexGridProps) {
  ...
  // Includes ONLY water cells (state 3) in groups of 3 or more.
  const waterClusters: Array<{ cells: HexCellData[] }> = [];
  if (themeId === 'fireice') {
    ...
    for (const cell of waterCells) {
      ...
      // Only spawn fish if the cluster has 3 or more connected water cells
      if (cluster.length >= 3) {
        waterClusters.push({ cells: cluster });
      }
    }
  }
